...
using aDVanceERP.Core.Modelos.Modulos.Inventario.Estadisticas;
...
namespace aDVanceERP.Core.Repositorios.Modulos.Inventario {
    public class RepoMovimiento : RepoEntidadBaseDatos<Movimiento, FiltroBusquedaMovimiento> {
        ...
        public override Movimiento? ObtenerPorId(object id) {
            var cacheKey = $"{NombreTabla}_Id_{id}";
            if (_cache.TryGetValue(cacheKey, out object? cachedObj) && cachedObj is Movimiento cachedEntity)
                return cachedEntity;

            var consulta = GenerarComandoObtener(FiltroBusquedaMovimiento.Id, out var parametros, id.ToString());

            parametros["@id_movimiento"] = Convert.ToInt64(id);

            var entidad = ContextoBaseDatos.EjecutarConsulta(consulta, parametros, MapearEntidad).FirstOrDefault().entidadBase;

            if (entidad != null)
                _cache.Set(cacheKey, entidad, TimeSpan.FromMinutes(5));
        ...
        protected override (Movimiento, List<IEntidadBaseDatos>) MapearEntidad(MySqlDataReader lectorDatos) {
            ...
            var entidad = new Movimiento() {
                ...
                FechaTermino = lectorDatos.IsDBNull(lectorDatos.GetOrdinal("fecha_termino")) ? DateTime.MinValue : Convert.ToDateTime(lectorDatos["fecha_termino"]),
            ...
            if (lectorDatos.FieldCount > 15) {
                entidad.NombreProducto = lectorDatos["nombre_producto"]?.ToString() ?? string.Empty;
                entidad.NombreAlmacenOrigen = lectorDatos["nombre_almacen_origen"]?.ToString() ?? string.Empty;
                entidad.NombreAlmacenDestino = lectorDatos["nombre_almacen_destino"]?.ToString() ?? string.Empty;
                entidad.NombreTipoMovimiento = lectorDatos["nombre_tipo_movimiento"]?.ToString() ?? string.Empty;
                entidad.EfectoMovimiento = Enum.TryParse<EfectoMovimientoEnum>(lectorDatos["efecto"].ToString(), out var efecto) ? efecto : EfectoMovimientoEnum.Ninguno;
            }

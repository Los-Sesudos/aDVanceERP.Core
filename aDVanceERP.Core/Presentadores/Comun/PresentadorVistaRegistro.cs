...
namespace aDVanceERP.Core.Presentadores.Comun {
    public abstract class PresentadorVistaRegistro<Vr, En, Re, Fb> : PresentadorVistaBase<Vr>, IPresentadorVistaRegistro<Vr, Re, En, Fb>
        ...
        protected virtual void EventoPostAdicionEntidad(Re repositorio, long id) { }

        protected virtual void EventoPostEdicionEntidad(Re repositorio, long id) { }
        ...
        private void RegistrarEditarEntidad(object? sender, EventArgs e) {
            ...
            try {
                ...
                if (entidad != null) {
                    ...
                    if (Vista.ModoEdicion)
                        EventoPostEdicionEntidad(_repositorio, entidad.Id);
                    else EventoPostAdicionEntidad(_repositorio, entidad?.Id ?? 0);

                    EntidadRegistradaActualizada?.Invoke(this, EventArgs.Empty);
                }
        ...
        protected void InvocarEntidadRegistradaActualizada() {
            EntidadRegistradaActualizada?.Invoke(this, EventArgs.Empty);
        }
